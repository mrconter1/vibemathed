...
import { HEADER_ICON_SHELL } from "@/lib/header-button";
...
export function DiscordLink() {
  return (
    <a
      ...
      // Blurple at rest, and the hover raises the border to match instead of
      // the usual accent blue: a button that changes hue on hover reads as two
      // different controls, and the point of the colour is that the mark is
      // recognisable before it is read.
      className={`inline-flex ${HEADER_ICON_SHELL} text-[var(--brand-discord)] transition-colors hover:border-[var(--brand-discord)]`}
    >
      {/* Discord's mark, in their blurple - the one glyph in this row that is
          not monochrome ink. It is a link off the site rather than a control
          on it, and the brand colour is what makes it findable at a glance,
          which is the entire job of a permanent header slot. */}
