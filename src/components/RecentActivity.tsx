...
function describe(a: SiteActivityView) {
  ...
  switch (a.type) {
    ...
    case "updated": {
      ...
      // A run of edits in one sitting reads as one line. The count goes on
      // the end rather than replacing the detail, so the most recent change
      // is still named and the rest are accounted for.
      const more = a.alsoChanged
        ? ` and ${a.alsoChanged} other ${a.alsoChanged === 1 ? "field" : "fields"}`
        : "";
      if (!hadOld && hasNew) {
        return (
          <>
