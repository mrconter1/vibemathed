...
async function main() {
  ...
  const EDITS: { field: string; key: string; value: unknown }[] = [
    ...
    { field: "Verification", key: "verification", value: "site-confirmed" },
    {
      ...
      value:
        `Reproduced by this site on 14 August 2026, with the submitter's referee code, witnesses and CNFs deliberately unread by the checker. All 19 instances regenerate from the vendored DD26 generator and match their pinned SHA-256 byte for byte. All 19 LOWER bounds were established independently: the five pattern families, both group actions and the monotone-embedding notion were implemented here from the DD26 paper's definitions alone, a witness coloring at $n-1$ was found for every value, and each witness was re-verified by a brute-force embedding search sharing no code with the encoder. On the UNSAT side, ${UNSAT_N} of 19 instances were re-solved to UNSATISFIABLE at the claimed $n$ on regenerated CNFs; the rest exceeded this site's solver budget (the instances are genuinely hard - the submitter's smallest took kissat nine minutes) and rest on the bundle's own certificates, whose 19 drat-trim logs all read "s VERIFIED" with parsed dimensions matching the regenerated CNFs exactly. Every cell with a DD26 conjecture matches that conjecture's formula, checked by hand, and the three K-family cells are implied by the two sibling theorem entries this site verified on 13 August. Not covered: no human peer review - produced and refereed by AI agents in one pipeline (six referee legs, each with its own encoder); the Lean file is a statement anchor with zero proofs, by design. Four further cells await their final referee leg and are, correctly, not claimed.`,
    },
...
The correction: your note names the overlap cells as (P3,K4), (P4,K2), (P4,K3). The bundle's actual K-cells are (P4,K6), (P3,K9), (P9,K3). The theorems imply all three real cells, so nothing breaks, but the names were wrong and the entry now records the right ones. The statement's "nineteen previously unknown" also overcounted - three of the nineteen are values your own sibling theorems already prove - so it now reads sixteen plus three confirmations.

What I checked, without reading your referees' code, witnesses or CNFs: all 19 instances regenerate and match their pinned hashes byte for byte (one wrinkle worth knowing: on Windows the pins only match after CRLF normalization). All 19 lower bounds I established independently - my own implementation of the five families, both group actions and the embedding notion from the paper's definitions, a witness at n-1 for every value, each re-verified by a brute-force check sharing no code with my own encoder. On the UNSAT side I re-solved ${UNSAT_N} of 19 at the claimed n on regenerated instances before my solver budget ran out - your warning that these are hard was accurate - and for the rest, your drat-trim logs all read VERIFIED with dimensions matching my regenerated CNFs exactly. I also hand-checked every conjectured cell against DD26's formulas: all agree.
